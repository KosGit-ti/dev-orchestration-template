...
configure_codex_sandbox() {
  ...
  if grep -Eq '^[[:space:]]*writable_roots[[:space:]]*=' "$code_config" 2>/dev/null; then
    ...
    awk -v ins="$inserts" '
      !done && /^[[:space:]]*writable_roots[[:space:]]*=[[:space:]]*\[/ {
        sub(/\[/, "[" ins)
        done=1
      }
      { print }
...
if [ "$UPDATE_ONLY" = true ]; then
  ...
  chmod +x "$SKILL_DIR/scripts/codex-bridge.js" 2>/dev/null || true
  install_windows_helpers
