...
remove_prejoin_registration() {
  local config="$TEAMS_DIR/$TEAM/config.json"
  [ -f "$config" ] || return 0

  local config_sql project_sql type_sql updated tmp_config remaining lock_dir
  lock_dir="$(agmsg_team_lock_acquire "$TEAM")"
  trap 'agmsg_team_lock_release "$lock_dir"' RETURN
  config_sql=$(printf '%s' "$config" | sed "s/'/''/g")
  project_sql=$(printf '%s' "$PROJECT" | sed "s/'/''/g")
  type_sql=$(printf '%s' "$AGENT_TYPE" | sed "s/'/''/g")
  agent_json_path="$.agents.\"$NAME\""
  ...
  updated=$(sqlite3 :memory: "
    ...
    agent(raw) AS (
      SELECT json_extract(json, '$agent_json_path') FROM cfg
    ),
    normalized(json) AS (
      SELECT CASE
        WHEN raw IS NULL OR raw = 'null' THEN json_object('registrations', json_array())
        WHEN json_type(json_extract(raw, '\$.registrations')) = 'array' THEN raw
        ELSE json_object(
          'registrations',
          json_array(json_object(
            'type', json_extract(raw, '\$.type'),
            'project', json_extract(raw, '\$.project')
          ))
        )
      END
      FROM agent
    ...
    filtered(json) AS (
      SELECT json_object(
        ...
        COALESCE((
          ...
          FROM normalized, json_each(json_extract(normalized.json, '\$.registrations'))
          WHERE NOT (
            json_extract(value, '\$.type') = '$type_sql'
    ...
    SELECT CASE
      WHEN json_array_length(json_extract(filtered.json, '\$.registrations')) = 0
        THEN json_remove(cfg.json, '$agent_json_path')
      ELSE json_set(cfg.json, '$agent_json_path', json(filtered.json))
    END
    FROM cfg, filtered;
  ")
...
JOIN_RESULT="$(mktemp "${TMPDIR:-/tmp}/agmsg-spawn-join.XXXXXX")"
AGMSG_RESOLVE_PROJECT=0 AGMSG_JOIN_RESULT_FILE="$JOIN_RESULT" "$SCRIPT_DIR/join.sh" "$TEAM" "$NAME" "$AGENT_TYPE" "$PROJECT" >/dev/null
PREJOINED=1
PREJOIN_ADDED=0
if grep -qx 'added_registration=true' "$JOIN_RESULT" 2>/dev/null; then
  PREJOIN_ADDED=1
fi
LAUNCH_SUCCEEDED=0
cleanup_prejoin_on_exit() {
  local status=$?
  rm -f "${JOIN_RESULT:-}" 2>/dev/null || true
  if [ "$PREJOINED" = "1" ] && [ "$PREJOIN_ADDED" = "1" ] && [ "$LAUNCH_SUCCEEDED" = "0" ]; then
    remove_prejoin_registration >/dev/null 2>&1 || true
  fi
  exit "$status"
}
trap cleanup_prejoin_on_exit EXIT
...
case "$AGENT_TYPE" in
  claude-code|copilot) ACTAS_PROMPT="/${CMD_NAME} actas ${NAME}" ;;
