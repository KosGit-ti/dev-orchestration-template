...
if command -v shasum >/dev/null 2>&1; then
  ...
  PROJECT_HASH="$(printf '%s' "$PROJECT" | sha1sum | awk '{print $1}')"
elif command -v openssl >/dev/null 2>&1; then
  PROJECT_HASH="$(printf '%s' "$PROJECT" | openssl sha1 | awk '{print $NF}')"
else
  echo "codex-monitor: shasum, sha1sum, or openssl is required to hash the project path" >&2
  exit 1
fi
