...
if [ "$FORCE" = "1" ]; then
  ...
  if [ -n "${_proj:-}" ] && [ -n "${_type:-}" ]; then
    AGMSG_RESET_TEAM="$TEAM" "$SCRIPT_DIR/reset.sh" "$_proj" "$_type" "$NAME" >/dev/null 2>&1 || true
  fi
...
case "$state" in
  free)
    ...
    if [ -f "$SPAWN_REC" ]; then
      echo "status=timeout name=$NAME team=$TEAM note=no-live-lock-placement-kept"
      echo "despawn: placement record is still available; retry with --force to tear it down." >&2
      exit 3
    fi
