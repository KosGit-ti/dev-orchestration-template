...
RESOLVED_PROJECT="$(agmsg_resolve_project "$PROJECT" "$TYPE")"
IDENTITIES=$("$SCRIPT_DIR/identities.sh" "$RESOLVED_PROJECT" "$TYPE")
...
MARKER_KEY="$(sha1_text "$TYPE"$'\t'"$RESOLVED_PROJECT"$'\t'"$IDENTITIES")"
MARKER="$SKILL_DIR/run/.lastcheck-$MARKER_KEY"
...
while IFS=$'\t' read -r team AGENT; do
  [ -n "$team" ] || continue
  [ -n "$AGENT" ] || continue
  agmsg_validate_agent_name "$AGENT" || continue
  agmsg_validate_team_name "$team" || continue
  ...
  if [ -n "$RESULT" ]; then
    ...
    OUTPUT+=$'\n'
    # Mark only the rows included in RESULT. A message arriving after the SELECT
    # must stay unread for the next hook turn.
    if [ -n "$IDS" ]; then
