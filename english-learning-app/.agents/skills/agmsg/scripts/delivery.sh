...
agmsg_command_match_sql() {
  local skill_dir_sql script_dir_sql
  skill_dir_sql=$(printf '%s/' "$SKILL_DIR" | sed "s/'/''/g")
  script_dir_sql=$(printf '%s/' "$SCRIPT_DIR" | sed "s/'/''/g")
  printf "instr(coalesce(json_extract(h.value, '\$.command'), ''), '%s') > 0 OR instr(coalesce(json_extract(h.value, '\$.command'), ''), '%s') > 0" "$skill_dir_sql" "$script_dir_sql"
}
            ...
            WHERE NOT EXISTS (
              SELECT 1 FROM json_each(json_extract(s.value, '\$.hooks')) AS h
	              WHERE instr(json_extract(h.value, '\$.command'), '$skill_dir_sql') > 0
	                 OR instr(json_extract(h.value, '\$.command'), '$script_dir_sql') > 0
            )) = 0 THEN
...
add_event_entry_file() {
  ...
  sql_path=$(sql_readfile_path "$path")
	  ...
	  local cmd_json
	  cmd_json=$(json_escape_string "$cmd")
	  local hook_inner="\"type\":\"command\",\"command\":\"$cmd_json\""
...
apply_settings() {
  ...
  case "$mode" in
    monitor)
      local ss se
      ss="$(shell_quote "$SKILL_DIR/scripts/session-start.sh") $(shell_quote "$type") $(shell_quote "$project")"
      se="$(shell_quote "$SKILL_DIR/scripts/session-end.sh")   $(shell_quote "$type") $(shell_quote "$project")"
      add_event_entry_file "$tmp_state" "SessionStart" "$ss" "$type"
      add_event_entry_file "$tmp_state" "SessionEnd"   "$se" "$type"
      ;;
    turn)
      local cmd
      cmd="$(shell_quote "$SKILL_DIR/scripts/check-inbox.sh") $(shell_quote "$type") $(shell_quote "$project")"
    ...
    both)
      ...
      ss="$(shell_quote "$SKILL_DIR/scripts/session-start.sh") $(shell_quote "$type") $(shell_quote "$project")"
      se="$(shell_quote "$SKILL_DIR/scripts/session-end.sh")   $(shell_quote "$type") $(shell_quote "$project")"
      st="$(shell_quote "$SKILL_DIR/scripts/check-inbox.sh")   $(shell_quote "$type") $(shell_quote "$project")"
      add_event_entry_file "$tmp_state" "SessionStart" "$ss" "$type"
      add_event_entry_file "$tmp_state" "SessionEnd"   "$se" "$type"
...
emit_monitor_directive() {
  ...
  local quoted_watch quoted_project quoted_type
  quoted_watch="$(shell_quote "$watch")"
  quoted_project="$(shell_quote "$project")"
  quoted_type="$(shell_quote "$type")"
...
AGMSG-DIRECTIVE: For this running session, invoke the Monitor tool now with:
  command: $quoted_watch $session_id $quoted_project $quoted_type
...
stop_codex_bridge() {
  local project="$1"
  local pairs team name key pidfile bpid killed=0
  pairs=$("$SCRIPT_DIR/identities.sh" "$project" codex 2>/dev/null || true)
  if [ -n "$pairs" ]; then
    while IFS=$'\t' read -r team name _rest; do
      [ -n "$team" ] && [ -n "$name" ] || continue
      key="$(sha1_text "$project	codex	$team	$name")"
      pidfile="$RUN_DIR/codex-bridge.$key.pid"
      [ -f "$pidfile" ] || continue
...
do_status() {
  ...
  if [ -n "$TYPE" ] && [ -n "$PROJECT" ]; then
    ...
    if [ "$TYPE" = "gemini" ] || [ "$TYPE" = "antigravity" ] || [ "$TYPE" = "copilot" ] || [ "$TYPE" = "opencode" ]; then
      ...
      if [ -f "$hf" ]; then
        local sql_hf match_sql
        sql_hf=$(sql_readfile_path "$hf")
        match_sql=$(agmsg_command_match_sql)
...
kill_all_watchers() {
  ...
  if [ -d "$RUN_DIR" ]; then
    for f in "$RUN_DIR"/watch.*.pid; do
      [ -f "$f" ] || continue
      local pid cmd meta meta_project
      pid=$(cat "$f" 2>/dev/null || echo "")
      meta="${f%.pid}.meta"
      if [ -n "$pid" ] && kill -0 "$pid" 2>/dev/null; then
        ...
        case "$cmd" in
          *"$SKILL_DIR/scripts/watch.sh"*)
            if [ -n "$project" ]; then
              [ -f "$meta" ] || continue
              IFS=$'\t' read -r meta_project _ < "$meta" || true
