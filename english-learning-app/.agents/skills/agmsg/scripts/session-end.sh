...
if [ "$TYPE" = "codex" ]; then
  for meta in "$RUN_DIR"/codex-bridge.*.meta; do
    [ -f "$meta" ] || continue
    meta_project=$(sed -n 's/^project=//p' "$meta" 2>/dev/null | head -1)
    meta_type=$(sed -n 's/^type=//p' "$meta" 2>/dev/null | head -1)
    [ "$meta_project" = "$PROJECT" ] && [ "$meta_type" = "$TYPE" ] || continue
    bridge_pid=$(sed -n 's/^pid=//p' "$meta" 2>/dev/null | head -1)
    if [ -n "$bridge_pid" ] && kill -0 "$bridge_pid" 2>/dev/null; then
      cmd=$(ps -o args= -p "$bridge_pid" 2>/dev/null || true)
      case "$cmd" in
        *"$SKILL_DIR/scripts/codex-bridge.js"*) kill "$bridge_pid" 2>/dev/null || true ;;
        *) ;;
      esac
    fi
    rm -f "${meta%.meta}.pid" "$meta" 2>/dev/null || true
  done
fi
