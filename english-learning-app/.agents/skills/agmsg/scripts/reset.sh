...
RESET_TEAM="${AGMSG_RESET_TEAM:-}"
if [ -n "$RESET_TEAM" ]; then
  agmsg_validate_team_name "$RESET_TEAM" || exit 1
fi
...
REMOVED=0
TOUCHED_TEAMS=0
CURRENT_TEAM_LOCK_DIR=""
cleanup_current_team_lock() {
  if [ -n "$CURRENT_TEAM_LOCK_DIR" ]; then
    agmsg_team_lock_release "$CURRENT_TEAM_LOCK_DIR"
  fi
}
trap cleanup_current_team_lock EXIT

for TEAM_CONFIG in "$TEAMS_DIR"/*/config.json; do
  [ -f "$TEAM_CONFIG" ] || continue
  TEAM_DIR="$(dirname "$TEAM_CONFIG")"
  TEAM_NAME="$(basename "$TEAM_DIR")"
  if [ -n "$RESET_TEAM" ] && [ "$TEAM_NAME" != "$RESET_TEAM" ]; then
    continue
  fi
  CURRENT_TEAM_LOCK_DIR="$(agmsg_team_lock_acquire "$TEAM_NAME")"
  CONFIG_ESCAPED=$(sed "s/'/''/g" "$TEAM_CONFIG")
  PROJECT_ESCAPED=$(printf '%s' "$PROJECT_PATH" | sed "s/'/''/g")
  AGENT_JSON_PATH="$.agents.\"$TARGET_AGENT\""

  CONFIG_PATH=$(printf '%s' "$TEAM_CONFIG" | sed "s/'/''/g")
  AGENT_JSON=$(sqlite3 :memory: \
    "SELECT json_extract(readfile('$CONFIG_PATH'), '$AGENT_JSON_PATH');")
  ...
  if [ "$REMAINING" -eq 0 ]; then
    UPDATED=$(sqlite3 :memory: \
      "SELECT json_remove(readfile('$CONFIG_PATH'), '$AGENT_JSON_PATH');")
  else
    UPDATED=$(sqlite3 :memory: \
      "SELECT json_set(readfile('$CONFIG_PATH'), '$AGENT_JSON_PATH', json('$FILTERED_ESCAPED'));")
  fi
