...
# [A-Za-z0-9.-] is percent-encoded byte-by-byte (UTF-8 safe, reversible).
# Underscore is encoded too because "__" is used as a filename separator.
...
actas_lock_gc_stale() {
  ...
  local f owner reclaim_dir current count=0
  for f in "$dir"/actas.*.session; do
    [ -f "$f" ] || continue
    owner="$(head -1 "$f" 2>/dev/null || true)"
    if [ -z "$owner" ] || ! actas_lock_sid_alive "$owner"; then
      reclaim_dir="${f}.reclaim.d"
      if mkdir "$reclaim_dir" 2>/dev/null; then
        current="$(head -1 "$f" 2>/dev/null || true)"
        if [ "$current" = "$owner" ] && { [ -z "$current" ] || ! actas_lock_sid_alive "$current"; }; then
          rm -f "$f"
          count=$((count + 1))
        fi
