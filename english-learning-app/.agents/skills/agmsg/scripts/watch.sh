...
METAFILE="$RUN_DIR/watch.$SESSION_ID.meta"
...
echo $$ > "$PIDFILE"
printf '%s\t%s\t%s\n' "$PROJECT_PATH" "$AGENT_TYPE" "$ACTIVE_NAME" > "$METAFILE" 2>/dev/null || true
  ...
  if [ -f "$DB" ]; then
    ...
    if [ -n "$ROWS" ]; then
      ...
      while IFS=$'\x1f' read -r id ts team from to body; do
        ...
        printf '%s | %s | %s → %s | %s\n' "$ts" "$team" "$from" "$to" "$body"
        DELIVERED_IDS="${DELIVERED_IDS:+$DELIVERED_IDS,}$id"
        LAST="$id"
      done <<< "$ROWS"
      if [ -n "$DELIVERED_IDS" ]; then
        agmsg_sqlite "$DB" \
          "UPDATE messages SET read_at = COALESCE(read_at, strftime('%Y-%m-%dT%H:%M:%SZ','now')) WHERE id IN ($DELIVERED_IDS);" \
          2>/dev/null || true
      fi
