...
└── mode_branch              ... Step 11: モード分岐（単発→PR完成報告 / 全プラン→自動マージ→次タスク）
```
...
| `pipeline.mode` | string | 動作モード | `"auto"` / `"full_plan"` / `"general"` / `"plan_revision"` |
...
| `agent.model` | string | 使用モデル名（実行時解決値。正本: ai/capability-registry.yml） | `"<resolved-model-id>"` |
