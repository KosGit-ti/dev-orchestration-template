...
9. PR の CI を監視する（失敗時は修正→再プッシュ、最大3回）
   ...
   - pending のままタイムアウト・無応答の場合：**PR 状態診断フロー**（後述）を実行して原因（コンフリクト・ブランチ遅延等）を特定・解消してから CI 監視に戻る
   - エスカレーション：3回試行しても解決できない場合は人間に報告して停止する
...
while review_iteration < 3 かつ Must/Should 指摘あり:
    ...
    9.5. プッシュ後 GitHub CI の再確認（必須）
       - `gh pr checks <PR_NUMBER>` でチェック一覧を取得し、全ステータスを確認する
       - **全チェックが passed / success** → 次のステップへ進む
       - **いずれかが failed** → Step 6 の修正フローに戻り、最大3回まで修正・再プッシュを試みる
       - **pending のまま変化なし（タイムアウト目安: 10分）または応答なし** →
