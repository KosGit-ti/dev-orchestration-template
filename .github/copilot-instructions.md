...
10. Copilot コードレビュー対応ループ（最大3回）
    - **Push 型アーキテクチャ**: Ruleset「Review new pushes」による自動トリガー（Remove→Re-add は廃止）
    - **GitHub App Token / PAT でプッシュ**（GITHUB_TOKEN では無限ループ防止により Copilot が発火しない）
    - **静的解析ファースト**: 修正後は必ず CI + get_errors を通過してからプッシュする
