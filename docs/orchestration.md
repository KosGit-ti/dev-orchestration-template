...
| 汎用リクエスト | 上記以外 | 改善提案・調査・リファクタリング等（PR を伴う場合は自動実行 Step 5-12 相当の品質管理＋PR フローを適用。copilot-instructions.md 参照） |
...
- **ループカウント**: 復旧時は Status ファイルのループカウントを引き継ぐ。復旧前後を合算して最大3回の制限を適用する
